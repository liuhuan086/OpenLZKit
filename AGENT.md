# AGENT.md — OpenLZKit 统一入口

> 这是 OpenLZKit 仓库面向 **AI 助手和开发者** 的统一入口文件。
...
5. **[docs/](docs/README.md)** — 治理方法论，从 [docs/README.md](docs/README.md) 索引进入。每个主题只有一个权威位置：
   ...
   实现某个领域前先读对应文档。
6. **`multi-cloud/<cloud>/docs/design.md`** — 动某朵云之前，先读该云的设计文档与模型文档。
...
└── .github/workflows/  # CI 检查（terraform fmt/validate/lint/policy/security）
```
...
5. **每个模块必须配套**：README、变量说明、输出说明、`examples/basic`、测试用例、安全注意事项。
6. **外科手术式改动**：只改与任务直接相关的内容，不顺手“优化”无关代码、注释或格式；发现无关死代码先提出、不擅自删。
7. **目标驱动**：先定义可验证的成功标准（fmt / validate / lint / policy / plan / 文档齐全），再循环到通过。
...
- [ ] 通过 `terraform fmt` / `validate` / lint / policy / `plan`
- [ ] 不破坏 state 兼容性
- [ ] 重大设计决策已记录 ADR

## 当前状态

项目处于 **MVP 阶段**：文档、目录骨架、测试策略与 CI 框架已就位；`multi-cloud/<cloud>/{modules,live}` 多为占位（`.gitkeep`），尚无落地 HCL。
继续实现时优先级为 **AWS 与阿里云**（见 [README.md](README.md) 路线图），并严格沿用上述标准。
