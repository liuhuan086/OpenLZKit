...
| `70-workload-onboarding` | [`modules/workload-onboarding`](modules/workload-onboarding) | ✅ 已实现（工作负载服务账号 + 团队 IAM + 标准标签模板） |
...
- 扩展 Organization Policy、SCC findings、IAM 过宽授权和标签类 Rego 用例。
- 增加 sandbox apply / rollback 脚本化证据。
- 完善 Shared VPC service project、WIF 和 project factory 企业流程示例。
